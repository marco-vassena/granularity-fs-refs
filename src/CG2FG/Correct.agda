...
mutual
  ...
  -- cg2fg ≈θ (⌞_⌟ᵀ {μ = μ} ⟨ ≈Σ , ≈μ ⟩) (New-FS {pc = pc} x pc⊑ℓ) with cg2fgᴾ _ pc ≈θ x
  -- ... | Id (⟨ ⌞ ℓ ⌟ ^ .ℓ , r ^ ℓ' ⟩ ^ _) ∧ Labeled ⊑pc (ℓ'⊑ℓ ↓ ≈r) ∧ x'
  --   = c ∧ ≈c ∧ c⇓
  --  where pc-⊔-ℓ = sym (ub pc⊑ℓ)
  --        c = ⟨ _ , FG.snocᴴ μ (r ^ ℓ) , (Refˢ FG.∥ μ ∥ᴴ) ^ pc ⟩ -- ℓ because ℓ' ⊑ ℓ
  --        c⇓ = New-FS
  --               (App Fun (UnId x' (sym (ub' ⊑pc))) (idem-⊔' pc)
  --                 (Taint pc-⊔-ℓ
  --                 (Fst (Var here (idem-⊔' pc)) pc-⊔-ℓ)
  ...
  cg2fg ≈θ ⌞ ⟨ ≈Σ , ≈μ ⟩ ⌟ᵀ (Write-FS {pc = pc} x₁ x₂ n∈μ ⊑₁ refl w) with cg2fgᴾ _ pc ≈θ x₁ | cg2fgᴾ _ pc ≈θ x₂
