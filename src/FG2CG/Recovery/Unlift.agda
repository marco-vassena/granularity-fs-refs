...
private
  ...
  module Aux where
    ...
    mutual
      ...
      unlift-≈ᴱ : ∀ {Γ Γ' c₁ c₂} {θ₁ θ₂ : FG.Env Γ} {θ₁' θ₂' : CG.Env Γ'} →
                    θ₁' C.≈ᴱ θ₂' →
                    Fg2Cgᵉ c₁ θ₁ θ₁' →
                    Fg2Cgᵉ c₂ θ₂ θ₂' →
                    θ₁ F.≈ᴱ θ₂
      ...
      unlift-≈ᴿ : ∀ {τ τ' p₁ p₂} {r₁ r₂ : FG.Raw τ} {r₁' r₂' : CG.Value τ'} →
                    r₁' C.≈ⱽ r₂' →
                    Fg2Cgᴿ p₁ r₁ r₁' →
                    Fg2Cgᴿ p₂ r₂ r₂' →
                    r₁ F.≈ᴿ r₂
      ...
      unlift-≈ⱽ : ∀ {τ τ' p₁ p₂} {v₁ v₂ : FG.Value τ} {v₁' v₂' : CG.Value (Labeled τ')} →
                    v₁' C.≈ⱽ v₂' →
                    Fg2Cgⱽ p₁ v₁ v₁' →
                    Fg2Cgⱽ p₂ v₂ v₂' →
                    v₁ F.≈ⱽ v₂
      ...
      unlift-≈ᴹ : ∀ {ℓ} {M₁ M₂ : FG.Memory ℓ} {M₁' M₂' : CG.Memory ℓ} →
                     M₁' C.≈ᴹ M₂' →
                     Fg2Cgᴹ M₁ M₁' →
                     Fg2Cgᴹ M₂ M₂' →
                     M₁ F.≈ᴹ M₂
...
-- Public memories.
unlift-≈ᴹ : ∀ {ℓ} {M₁ M₂ : FG.Memory ℓ} → ⟪ M₁ ⟫ᴹ C.≈ᴹ ⟪ M₂ ⟫ᴹ → M₁ F.≈ᴹ M₂
unlift-≈ᴹ ⟪M₁≈M₂⟫ = Aux.unlift-≈ᴹ ⟪M₁≈M₂⟫ (mkFg2Cgᴹ _) (mkFg2Cgᴹ _)

-- Memories.
unlift-≈⟨_⟩ᴹ : ∀ {ℓ} {M₁ M₂ : FG.Memory ℓ} (x : Dec (ℓ ⊑ A)) → ⟪ M₁ ⟫ᴹ C.≈⟨ x ⟩ᴹ ⟪ M₂ ⟫ᴹ → M₁ F.≈⟨ x ⟩ᴹ M₂
unlift-≈⟨ yes p ⟩ᴹ ⟪M₁≈M₂⟫ = unlift-≈ᴹ ⟪M₁≈M₂⟫
unlift-≈⟨ no ¬p ⟩ᴹ _ = tt

-- Stores.
unlift-≈ˢ : ∀ {Σ₁ Σ₂} → ⟪ Σ₁ ⟫ˢ C.≈ˢ ⟪ Σ₂ ⟫ˢ → Σ₁ F.≈ˢ Σ₂
...
unlift-≈ᴵ : ∀ {τ Γ} {c₁ c₂ : IConf Γ τ} → (pc : Label) → ⟪ c₁ ⟫ᴵ pc C.≈ᴵ ⟪ c₂ ⟫ᴵ pc → c₁ F.≈ᴵ c₂
unlift-≈ᴵ pc ⟨ ≈ˢ , refl , term-≡ ⟩ = ⟨ unlift-≈ˢ ≈ˢ , inj-⟪ term-≡ ⟫ᴱ ⟩

-- Final configurations.
unlift-≈ᶜ : ∀ {pc τ} {c₁ c₂ : FG.FConf τ} →
            let ⟨ _ , _ ^ ℓ₁ ⟩ = c₁
                ⟨ _ , _ ^ ℓ₂ ⟩ = c₂ in
                pc ⊑ ℓ₁ →
                pc ⊑ ℓ₂ →
