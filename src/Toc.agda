...
module §2 where
  ...
  open import FG hiding (_⊆_ ; _×_)

  -- "The label of the result is at least as sensitive as the program counter"
  property₁ : ∀ {Σ Σ' μ μ' Γ τ pc r ℓ} {e : Expr Γ τ} {v : Value τ} {θ : Env Γ} →
               ⟨ Σ , μ , e ⟩ ⇓⟨ θ , pc ⟩ ⟨ Σ' , μ' , r ^ ℓ ⟩ →
               pc ⊑ ℓ
  property₁ = step-⊑
  ...
  -- L-equivalence
  figure₅ : Set
  figure₅ = ∀ {β : Bij} {τ : Ty} {v₁ v₂ : Value τ} → v₁ ≈ⱽ⟨ L , β ⟩ v₂

  open import Data.Product renaming (_,_ to _∧_)

  -- λ^FG TINI
  theorem₁ : ∀ {τ Γ θ₁ θ₂ pc β} {c₁ c₂ : IConf Γ τ} {c₁' c₂' : FConf τ} →
             Valid-Inputs c₁ θ₁ →
             Valid-Inputs c₂ θ₂ →
             c₁ ⇓⟨ θ₁ , pc ⟩ c₁' →
             c₂ ⇓⟨ θ₂ , pc ⟩ c₂' →
             ...
             θ₁ ≈ᴱ⟨ L , β ⟩ θ₂ →
             ∃ (λ β' → β ⊆ β' × c₁' ≈ᶜ⟨ L , β' ⟩ c₂')
  theorem₁ valid₁ valid₂ = tini {{valid₁}} {{valid₂}}
  ...
  theorem₁′ : TINI λ^FG
  theorem₁′ {A = A} isV₁ isV₂ c₁⇓ c₂⇓ c₁≈c₂ (θ₁≈θ₂ ∧ refl) = tini {{isV₁}} {{isV₂}} c₁⇓ c₂⇓ c₁≈c₂ θ₁≈θ₂
...
module §3 where
  ...
  -- as the intial value."
  property₂ : ∀ {Γ τ Σ Σ' μ μ' pc pc' v} {θ : Env Γ} {t : Thunk Γ (LIO τ)} →
                    ⟨ Σ , μ , pc , t ⟩ ⇓⟨ θ ⟩ ⟨ Σ' , μ' , pc' , v ⟩ →
                    pc ⊑ pc'
  property₂ = step-⊑

  property₂′ : ∀ {Γ τ Σ Σ' μ μ' pc pc' v} {θ : Env Γ} {e : Expr Γ (LIO τ)} →
  ...
  -- L-equivalence
  figure₉ : Set
  figure₉  = ∀ {τ β} {v₁ v₂ : Value τ} → v₁ ≈⟨ β ⟩ⱽ v₂
  ...
  -- λ^CG TINI
  theorem₂ : TINI λ^CG
  theorem₂ {A = A} valid₁ valid₂ = tiniᶠ {{valid₁}} {{valid₂}}
...
module §4 where
  ...
  theorem₃ : ∀ {Σ Σ' μ μ' Γ τ pc} {e : FG.Expr Γ τ} {v : FG.Value τ} {θ : FG.Env Γ} →
             ⟨ Σ , μ , e ⟩ ⇓⟨ θ , pc ⟩ ⟨ Σ' , μ' , v ⟩ →
  ...
  lemma-4·2 : ∀ {Γ τ β} {c₁ c₂ : FG.IConf Γ τ} pc →
  ...
  lemma-4·3 : ∀ {τ pc β} {c₁ c₂ : FG.FConf τ} →
                     let ⟨ Σ₁ , μ₁ , r₁ ^ ℓ₁ ⟩ = c₁
                         ⟨ Σ₂ , μ₂ , r₂ ^ ℓ₂ ⟩ = c₂ in
                     pc ⊑ ℓ₁ →
                     pc ⊑ ℓ₂ →
...
module §5 where
  ...
  definition₁ : ∀ {Γ τ Σ₁ Σ₂ μ₁ μ₂ pc} {e : CG.Expr Γ (LIO τ)} →
                  ⟨ Σ₁ , μ₁ , ⟦ e ⟧ᴱ FG.∘ Id （） ⟩ ↓≈ᴵ ⟨ Σ₂ , μ₂ , pc , e ⟩ ⇔ FG.⟨ Σ₁ , μ₁ ⟩ ↓≈ᴾ CG.⟨ Σ₂ , μ₂ ⟩
  definition₁ = equivalence ⌜_⌝ᴵ ⌞_⌟ᴵ

  definition₁′ : ∀ {Γ τ Σ₁ Σ₂ μ₁ μ₂ pc} {t : CG.Thunk Γ (LIO τ)} →
                   ⟨ Σ₁ , μ₁ , ⟦ t ⟧ᵀ ⟩ ↓≈ᵀ ⟨ Σ₂ , μ₂ , pc , t ⟩ ⇔ FG.⟨ Σ₁ , μ₁ ⟩ ↓≈ᴾ CG.⟨ Σ₂ , μ₂ ⟩
  definition₁′ = equivalence ⌜_⌝ᵀ ⌞_⌟ᵀ

  definition₁′′ : ∀ {τ Σ₁ Σ₂ μ₁ μ₂ pc} {v : CG.Value τ} {r : FG.Raw ⟦ τ ⟧ᵗ} →
                    ⟨ Σ₁ , μ₁ , r ^ pc ⟩ ↓≈ᶜ ⟨ Σ₂ , μ₂ , pc , v ⟩ ⇔ (FG.⟨ Σ₁ , μ₁ ⟩ ↓≈ᴾ CG.⟨ Σ₂ , μ₂ ⟩ × (r ↓≈⟨ pc ⟩ᴿ v))
  definition₁′′ = equivalence ( λ { ⟨ ≈ᴾ , r≈v ⟩ → ≈ᴾ ∧ ⌞ r≈v ⌟ᴿ }) (uncurry ⟨_,_⟩)
  ...
  lemma-5·2 : ∀ {Γ τ Σ μ pc} {θ : CG.Env Γ} {θ' : FG.Env ⟦ Γ ⟧ᶜ} {e : CG.Expr Γ τ} {v : CG.Value τ} →
                θ' ↓≈⟨ pc ⟩ᵉ θ →
                e ⇓ᴾ⟨ θ ⟩ v →
  ...
  lemma-5·3 : ∀ {Γ τ β} {c₁ c₂ : CG.EConf Γ (LIO τ)} →
                  c₁ CG.≈ᴵ⟨ L , β ⟩ c₂ →
                  ⟦ c₁ ⟧ᴵ FG.≈ᴵ⟨ L , β ⟩ ⟦ c₂ ⟧ᴵ
  lemma-5·3 = lift-≈ᴵ

  -- Unlifting L-equivalence from target to source
  lemma-5·4 : ∀ {pc τ β} {v₁ v₂ : FG.Value ⟦ τ ⟧ᵗ} {v₁' v₂' : CG.Value τ} →
                  pc ⊑ L →
                  v₁ FG.≈ⱽ⟨ L , β ⟩ v₂ →
                  v₁ ↓≈⟨ pc ⟩ⱽ v₁'  →
                  v₂ ↓≈⟨ pc ⟩ⱽ v₂'  →
  ...
  lemma-5·4′ : ∀ {pc τ β} {r₁ r₂ : FG.Raw ⟦ τ ⟧ᵗ} {v₁' v₂' : CG.Value τ} →
                  pc ⊑ L →
                  r₁ FG.≈ᴿ⟨ L , β ⟩ r₂ →
                  r₁ ↓≈⟨ pc ⟩ᴿ v₁'  →
                  r₂ ↓≈⟨ pc ⟩ᴿ v₂'  →
  ...
  lemma-5·4′′ : ∀ {pc Γ β} {θ₁ θ₂ : FG.Env ⟦ Γ ⟧ᶜ} {θ₁' θ₂' : CG.Env Γ} →
                  pc ⊑ L →
                  θ₁ FG.≈ᴱ⟨ L , β ⟩ θ₂ →
                  θ₁ ↓≈⟨ pc ⟩ᵉ θ₁'  →
                  θ₂ ↓≈⟨ pc ⟩ᵉ θ₂'  →
  ...
  lemma-5·4′′′ : ∀ {ℓ β} {M₁' M₂'} {M₁ M₂ : FG.Memory ℓ} →
                 M₁ FG.≈ᴹ⟨ L , β ⟩ M₂ →
                 M₁ ↓≈ᴹ M₁' →
                 M₂ ↓≈ᴹ M₂' →
                 M₁' CG.≈ᴹ⟨ L , β ⟩ M₂'
  ...
  lemma-5·5 : ∀ {τ τ' β} {c₁' c₂' : CG.FConf τ} {c₁ c₂ : FG.FConf τ'} →
              c₁ FG.≈ᶜ⟨ L , β ⟩ c₂ →
              c₁ ↓≈ᶜ c₁' →
              c₂ ↓≈ᶜ c₂' →
              c₁' CG.≈ᶜ⟨ L , β ⟩ c₂'
  ...
  theorem₆ : TINI λ^CG
  theorem₆ {A = A} valid₁ valid₂ = R.tini-via-fg {{ valid₁ }} {{ valid₂ }}
